...
@login_required(login_url='login')
def fileDirectory(request):
    media_path = settings.MEDIA_ROOT
    files = [item for item in listdir(media_path)]
    ...
    for parent in files:
        ...
        if not isfile(join(media_path, parent)):
            childList = []

            for child in listdir(join(media_path, parent)):
                childList.append(child)

            parentList.append(childList)

        dirList.append(parentList)

    context = {'dirList': dirList}
    return render(request, 'base/fileDirectory.html', context)
...
def newDirectory(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                mkdir(join(media_path, request.POST.get('name')))
    ...
    context = {'form': form}
    return render(request, 'base/newDirectory.html', context)

@login_required(login_url='login')
def deleteDirectory(request, dir):
    media_path = settings.MEDIA_ROOT

    if request.method == 'POST':
        shutil.rmtree(join(media_path, dir), ignore_errors=True)
        messages.success(request, 'Directory deleted successfully!')

        return redirect('file-directory')

    return render(request, 'base/delete.html', {'obj': dir})

@login_required(login_url='login')
def newFile(request, dir):
    media_path = settings.MEDIA_ROOT
    parent_path = join(media_path, dir)
    form = FileForm()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                with open(join(parent_path, fileToUpload.name), 'wb+') as writePath:
                    for chunk in fileToUpload.chunks():
                        writePath.write(chunk)
    ...
    context = {'form': form}
    return render(request, 'base/newFile.html', context)

@login_required(login_url='login')
def deleteFile(request, dir, fileName):
    media_path = settings.MEDIA_ROOT
    parent_path = join(media_path, dir)

    if request.method == 'POST':
        remove(join(parent_path, fileName))
        messages.success(request, 'File deleted successfully!')

        return redirect('file-directory')

    return render(request, 'base/delete.html', {'obj': fileName})
